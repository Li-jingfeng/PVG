...
def evaluation(iteration, scene : Scene, renderFunc, renderArgs, env_map=None):
    ...
    for config in validation_configs:
        ...
        if True:
            ...
            # 生成-1，1的离散值，给到pose
            left2right = torch.linspace(-1, 1, nums_frames)
            extrinsics = []
            for i in range(nums_frames):
                extrinsic = copy.deepcopy(tmp_pose)
                extrinsic[0, 3] += left2right[i]
            ...
            for i in range(len(extrinsics)):
                ...
                # add timestamp(change lane应该是同一个时间戳)
            ...
            imageio.mimsave(os.path.join(outdir, f"{config['name']}_lane_change_video.mp4"), pred_video, fps=5)
